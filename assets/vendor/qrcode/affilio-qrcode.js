/**
 * Dreamax Affiliates QR renderer.
 *
 * QR encoding is based on "QRCode for JavaScript" by Kazuhiko Arase,
 * Copyright (c) 2009, licensed under the MIT License. The CommonJS source
 * was wrapped for browser use and an SVG renderer was added by Dreamax Affiliates.
 * See the "Third-party notices" section in README.md, and LICENSE in this folder.
 */
( function ( global ) {
	'use strict';
...
module.exports = QRCode;
	...
	function normalize( request ) {
		return request === '.' ? './index' : request.replace( /\.js$/, '' );
	}
	...
	function createSvg( value, options ) {
		...
		var quietZone = Number.isFinite( settings.quietZone ) ? Math.max( 4, Math.floor( settings.quietZone ) ) : 4;
		var moduleCount = matrix.length;
		var viewSize = moduleCount + quietZone * 2;
		var dark = settings.dark || '#111111';
		...
		return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + viewSize + ' ' + viewSize + '" shape-rendering="crispEdges" role="img" aria-label="QR code">' +
	...
	function render( element, value, options ) {
		...
		element.innerHTML = createSvg( value, options );
		return true;
	}

	function downloadSvg( value, filename, options ) {
		var svg = createSvg( value, options );
		var blob = new Blob( [ svg ], { type: 'image/svg+xml;charset=utf-8' } );
		var url = global.URL.createObjectURL( blob );
		var link = global.document.createElement( 'a' );
		link.href = url;
		link.download = filename || 'affiliate-qr-code.svg';
